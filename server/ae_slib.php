...
# např. kolik_1_2_5(dosp,'je $ dospělý,jsou $ dospělí,je $ dospělých')
function kolik_1_2_5($kolik,$tvary) {
  $tvar= explode(',',$tvary);
  $tvar= $kolik>4 ? $tvar[2] : ($kolik>1 ? $tvar[1] : ($kolik>0 ? $tvar[0] : $tvar[2]));
  return strstr($tvar,'$') ? str_replace('$',$kolik,$tvar) : "$kolik $tvar";
}
# -------------------------------------------------------------------------------------- lorem ipsum
