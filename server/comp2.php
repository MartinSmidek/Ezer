...
//   $debugger= true;
  ...
  require_once("ezer3/server/comp2.php");
...
# name :: id ( '.' id )*
function gen_name($name,$pars,$vars,$first,$c=null,$nargs=null) {  #trace();
...
//                                                 display("code: p($ipar) pro {$c->par}");
  ...
  case 'ops':                                                // call+ [ name ]
    ...
    for ($i= 0; $i<count($c->body); $i++) {
      ...
      if ( $ci->expr=='call' ) {
        ...
        $cc= gen_name($ci->name,$pars,$vars,false,$ci);
      }
      $code[$i]= $cc;
    }
    break;
  case 'name':                                               // id ( '.' id )*
    ...
    $code= gen_name($c->name,$pars,$vars,true,$c);
    break;
  case 'call':
    $code= array();
    $npar= count($c->par);
