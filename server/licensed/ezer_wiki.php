...
class WikiParser {
	...
	function WikiParser() {
		...
		$this->first_h1 = ''; //080219ms first header
		$this->first_h2 = ''; //080219ms first header
	}
	...
	function handle_sections($matches) {
		...
		$this->stop = true;
		if ( !$this->first_h1 && $level==1 ) $this->first_h1 = $content; //080219ms first header
		if ( !$this->first_h2 && $level==2 ) $this->first_h2 = $content; //080219ms first header
	...
	function handle_preformat($matches,$close=false) {
		...
		$this->preformat = true;
		$output .= htmlentities($matches[1],ENT_NOQUOTES,'UTF-8'); //080303gn //100505gn
	...
	function handle_image($href,$title,$options) {
		...
		foreach ($options as $k=>$option) {
			switch($option) {
				case 'frame':
					$imagetag = sprintf(
//080221ms      				'<div style="float: right; background-color: #F5F5F5; border: 1px solid #D0D0D0; padding: 2px">'.
						'<div align="center" style="border: 1px solid #D0D0D0; padding: 2px">'.
						'%s'.
						'<div align="center">%s</div>'.  //080221ms
						'</div>',
						$imagetag,
						$title
					);
				...
				case 'right':
					...
					break;
				case 'left':        //080225ms - added lefthand image
					$imagetag = sprintf(
	...
	function emphasize_off() {
		$output = "";
		if ( count($this->emphasis) )        //080219ms
...
//			'link'=>'(\[\[((.*?)\:)?(.*?)(\|(.*?))?\]\]([a-z]+)?)',
		...
		foreach ($line_regexes as $func=>$regex) {
			if (preg_match("/$regex/ui",$line,$matches)) {
				$called[$func] = true;
				$func = "handle_".$func;
		...
		if (!$this->stop_all) {
			...
			foreach ($char_regexes as $func=>$regex) {
				$line = preg_replace_callback("/$regex/iu",array(&$this,"handle_".$func),$line);
				if ($this->stop) break;
			}
		}
...
";
	...
	function parse($text,$title="") {
		...
		$text = preg_replace_callback('/<nowiki>([\s\S]*)<\/nowiki>/iu',array(&$this,"handle_save_nowiki"),$text);

		$lines = explode("\n",$text);
		
		if (preg_match('/^\#REDIRECT\s+\[\[(.*?)\]\]$/u',trim($lines[0]),$matches)) {
		...
		foreach ($lines as $k=>$line) {
			...
			$output .= "$line ";   //080225ms - added missing space after end of line
                    }
		}

		$output = preg_replace_callback('/<nowiki><\/nowiki>/iu',array(&$this,"handle_restore_nowiki"),$output);
